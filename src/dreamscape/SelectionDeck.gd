...
func initiate_card_removal(cost: int = 0, cost_type: String = 'mastery') -> void:
	operation_cost = cost
...
func initiate_card_progress(cost: int = 0, cost_type: String = 'mastery', amount := 1) -> void:
	...
	if operation != "progress":
		_clear_existing_cache()
	operation = "progress"
	_deck_operation_name.text = "Progress Card for %s" % [amount]
	_display()

# Initiates an operation for card selection
func initiate_card_selection(cost: int = 0, cost_type: String = 'mastery') -> void:
	operation_cost = cost
	operation_cost_type = cost_type
